      ...
      .then((infractions: any[]) => {
        if (infractions?.length > 0) {
          ...
          infractions.forEach(inf => {
            ...
            if (!counts[key]) { counts[key] = 0; monthOrder.push(key) }
            counts[key]++
          })
          monthOrder.sort()
          setData(monthOrder.map(key => {
        ...
        {loading ? (
          ...
          <ResponsiveContainer width="100%" height={280}>
            <AreaChart data={data} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="infractions" stroke="#3b82f6" strokeWidth={2.5}
                fill="url(#colorInfractions)" name="Infractions"
      ...
      .then((infractions: any[]) => {
        if (infractions?.length > 0) {
          ...
          infractions.forEach(inf => {
            const d = new Date(inf.date)
            const key = `${d.getFullYear()}-${String(d.getMonth()).padStart(2, '0')}`
            if (!monthData[key]) { monthData[key] = { critical: 0, high: 0, medium: 0, low: 0 }; monthOrder.push(key) }
            const sev = inf.severity as keyof typeof tots
            if (tots[sev] !== undefined) { monthData[key][sev]++; tots[sev]++ }
          })
          monthOrder.sort()
          setData(monthOrder.map(key => {
            const [year, month] = key.split('-')
          ...
          <div className="space-y-4">
            ...
            <ResponsiveContainer width="100%" height={160}>
              <BarChart data={data} margin={{ top: 5, right: 5, left: -15, bottom: 0 }}>
                ...
                <Tooltip contentStyle={tooltipStyle} labelStyle={tooltipLabelStyle} itemStyle={tooltipItemStyle} cursor={{ fill: "rgba(148,163,184,0.08)" }} />
                {severityConfig.map(s => (
                  <Bar key={s.key} dataKey={s.key} stackId="severity" name={s.label} fill={s.color}
                    radius={s.key === "low" ? [3, 3, 0, 0] : [0, 0, 0, 0]} />
                ))}
              </BarChart>
            </ResponsiveContainer>
          </div>
  ...
  useEffect(() => {
    fetch(`/api/infractions?dateFrom=${getDateLimit12Months()}`, { credentials: 'include' })
      ...
      .then((infractions: any[]) => {
        if (infractions) {
          const counts: Record<string, number> = {}
          infractions.forEach(inf => { counts[inf.type] = (counts[inf.type] || 0) + 1 })
          setData(
            Object.keys(counts)
